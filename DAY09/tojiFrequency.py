...
# 토지 파일을 읽어서 워드 클라우드와 막대 그래프 그리기
import numpy as np
...
class Visualization:
    ...
    def makeWordCloud(self): # 워드 클라우드
        ...
        alice_coloring = np.array(Image.open(alice_color_file)) # 이미지를 numpy 배열로 바꿉니다.
        wordcloud = WordCloud(font_path='malgun.ttf', mask=alice_coloring, relative_scaling=0.2, background_color='lightyellow')
...
# token : 더 이상 나눌 수 없는 가장 기본적인 언어 요소
import nltk
